...
from UniBasis_prop import UniBasisProp
...
class Model(nn.Module):
    ...
    def get_embedding(self, edge_index, feat):
        h1 = self.encoder(x=feat, edge_index=edge_index, highpass=True, update_tau=True)
        h2 = self.encoder(x=feat, edge_index=edge_index, highpass=False, update_tau=True)
    ...
    def forward(self, edge_index, feat, shuf_feat):
        ...
        h1 = self.encoder(x=feat, edge_index=edge_index, highpass=True, update_tau=True)
        h2 = self.encoder(x=feat, edge_index=edge_index, highpass=False, update_tau=True)
        # negative (do NOT update τ̂)
        h3 = self.encoder(x=shuf_feat, edge_index=edge_index, highpass=True, update_tau=False)
        h4 = self.encoder(x=shuf_feat, edge_index=edge_index, highpass=False, update_tau=False)
...
class ChebNetII(torch.nn.Module):
    ...
    def forward(self, x, edge_index, highpass=True,update_tau=True):

        if self.dprate == 0.0:
            x = self.prop1(x, edge_index, highpass=highpass, update_tau=update_tau)
        else:
